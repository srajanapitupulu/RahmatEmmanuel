...
protocol DelegatePSCornerService: NSObjectProtocol {
  func psCornerServiceResponse(newsData data:[News])
}

class PsCornerService: NSObject {
  
  var service: Service!
  var delNewsService: AnyObject!
  
  //MARK: Initialization
  init(delNewsService: DelegatePSCornerService) {
  ...
  func getNewsData() {
    self.service.callGETRestAPI(withURL: Constants.API_PS_JOSHUA)
  }
}

extension PsCornerService:DelegateServiceCall {
